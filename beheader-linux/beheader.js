...
// Convert JPG to PNG (32bpp) for the script
await $`"C:\\Program Files\\ImageMagick-7.1.2-Q16-HDRI\\magick.exe" "${image}" -define png:color-type=6 -depth 8 -alpha on -strip "${tmpPng}"`;
...
try {
  ...
  await $`mp4edit.exe --replace ftyp:"${tmp + ".atom"}" "${tmp + "0.mp4"}" "${tmp + "1.mp4"}"`;
  ...
  await $`mp4edit.exe --insert skip:"${tmp + ".atom"}" "${tmp + "1.mp4"}" "${tmp + "2.mp4"}"`;
  ...
  await Bun.write(atomFile, ftypBuffer);
  await $`mp4edit.exe --replace ftyp:"${tmp + ".atom"}" "${tmp + "2.mp4"}" "${output}"`;
  ...
  if (zip.length > 0) {
    ...
    for (const curr of zip) {
      await $`7z x "${curr}" -o"${tmp}dir" -y`.quiet();
    }
    // Create archive from temporary directory
    // 1️⃣ Create ZIP from the temporary directory
    await $`7z a -tzip "${tmp}.zip" "${tmp}dir\\*" -y`.quiet();

    // 2️⃣ Append the ZIP file to the end of the output
    await fs.appendFile(output, await Bun.file(tmp + ".zip").bytes());

    // 3️⃣ Optional: "zip -A" equivalent — recreate the ZIP for offset fix
    // This step can be skipped if exact alignment is not critical
    await $`7z a -tzip "${tmp}_fixed.zip" "${output}" -y`.quiet();
    await fs.rename(tmp + "_fixed.zip", output);
